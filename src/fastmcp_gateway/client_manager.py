...
class UpstreamManager:
    ...
    max_schema_depth:
        ...
        call. Defaults to
        :data:`~fastmcp_gateway.sanitize.DEFAULT_MAX_SCHEMA_DEPTH` (5),
        matching prior behaviour. Must be between ``1`` and
        :data:`~fastmcp_gateway.sanitize.MAX_ALLOWED_SCHEMA_DEPTH`
        (``50``) inclusive; validated here at construction time (not
        deferred to the first populate call) so a caller that
        constructs ``UpstreamManager`` directly -- bypassing
        ``GatewayServer`` -- gets the same fail-fast guarantee.
    """
