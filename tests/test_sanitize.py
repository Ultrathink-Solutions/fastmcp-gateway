...
class TestOptionalNullDepthTransparency:
    ...
    def test_rejects_pathological_optional_chain_without_recursion_error(self) -> None:
        ...
        # Stack safety is asserted directly, because it is the property that
        # holds independently of the cap: the traversal must RETURN a finite
        # depth rather than exhaust the stack. Asserting only the raise would
        # tie this shield to the cap's current value.
        assert _schema_depth(schema) < 500
    ...
    def test_rejects_thousands_deep_optional_chain_without_recursion_error(self) -> None:
        ...
        for _ in range(3000):
            deep = {"anyOf": [deep, {"type": "null"}]}
        schema = {"type": "object", "properties": {"foo": deep}}
        assert _schema_depth(schema) < 3000
        with pytest.raises(SchemaValidationError):
            validate_input_schema(schema)
